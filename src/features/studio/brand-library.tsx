...
export function BrandLibrary({ assets, search, selected, onSelect }: { assets: Asset[]; search: string; selected: BrandTab; onSelect: (tab: BrandTab) => void }) {
  ...
  return <section className="library-section" aria-label="Brand library">
    ...
    {selected === "Overview" && <div className="brand-system"><section className="brand-showcase"><div className="brand-large-logo"><img src="/assets/brand/logo.png" alt="StatOz brand mark"/><span>StatOz<span>.</span></span></div><div><div className="section-caption">THE VISUAL SIGNATURE</div><h2>Competitive energy.<br/>Deliberate restraint.</h2><p>Dark navy. Clear hierarchy. A flash of cyan. Outlined cut-corner panels, purposeful motion, and the unmistakable StatOz mark.</p><Tag>BRAND SNAPSHOT · 15 SEP 2026</Tag></div></section><OverviewPrinciples/><p className="muted-note">The same guide drives the statoz-design-system assistant skill. Download tokens, fonts and the logo from Assets › Design kit.</p></div>}
    {selected === "Brand assets" && <AssetGrid assets={brandAssets} empty={query ? `No brand assets match “${search.trim()}”.` : "Registered StatOz brand artwork will appear here."}/>}
