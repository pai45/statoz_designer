...
export function Studio() {
  ...
  const openProject = useCallback((value: ProjectEnvelope) => { setOpened(value); window.history.replaceState(null, "", `?project=${value.project.id}`); }, []);
  useEffect(() => {
  ...
  const closeEditor = () => { setOpened(null); window.history.replaceState(null, "", "/"); void refresh({ projects: true }); };
  ...
  return <>
    ...
    <dialog ref={mediaDialog} className="modal media-preview-modal" onCancel={() => setPreviewJob(null)}><div className="modal-heading"><h2>{previewJob?.projectName}</h2><Button variant="ghost" aria-label="Close media preview" onClick={() => setPreviewJob(null)}><Icon name="close"/></Button></div>{previewJob && (previewJob.outputType === "mp4" ? <video controls src={`/api/exports/${previewJob.id}/file`} autoPlay/> : <img src={["zip", "pdf", "pptx"].includes(previewJob.outputType) ? `/api/exports/${previewJob.id}/poster` : `/api/exports/${previewJob.id}/file`} alt={previewJob.projectName}/>)}{previewJob && ["zip", "pdf", "pptx"].includes(previewJob.outputType) && <p className="muted-note">Cover preview. Download the finished {previewJob.outputType.toUpperCase()} to inspect every slide.</p>}</dialog>
