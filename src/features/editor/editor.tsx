...
import { durationOf, formats, isLineArt, sceneAt, sports, type Asset, type Format, type ProjectEnvelope, type RenderJob, type Scene } from "@/domain/project";
...
import { AppCreativeFields } from "./app-creative-fields";
import { appCreativeReadiness } from "@/domain/app-creatives";
import { Preview, Thumbnail } from "./preview";
import { useEditor } from "./use-editor";

export function Editor({ initial, assets, onClose, onOpen, onExports, notify, projectFolder }: { initial: ProjectEnvelope; assets: Asset[]; onClose: () => void; onOpen: (p: ProjectEnvelope) => void; onExports: () => void; notify: (m: string) => void; projectFolder: string }) {
  ...
  return <div className="editor">
    ...
    <aside className="inspector"><><div className="inspector-tabs">{(["Content", "Brief"] as typeof panel[]).map(p => <button key={p} onClick={() => setPanel(p)} aria-pressed={p === panel} className={p === panel ? "active" : ""}>{p}</button>)}</div><div className="inspector-content">
      {panel === "Content" && <>{!isStoreIcon && <><div className="section-caption">{isVideo ? `SCENE ${visibleIndex + 1}` : `PAGE ${visibleIndex + 1}`} / CONTENT</div><InputField label="Eyebrow" value={scene.eyebrow} onChange={e => updateScene({ eyebrow: e.target.value })}/><label className="field"><span>Headline</span><textarea rows={3} value={scene.headline} onChange={e => updateScene({ headline: e.target.value })}/><small>Line breaks are preserved in your design.</small></label><label className="field"><span>Supporting copy</span><textarea rows={3} value={scene.body} onChange={e => updateScene({ body: e.target.value })}/></label>{isPitch ? <PitchSlideFields scene={scene} onChange={updateScene}/> : <InputField label="Call to action" value={scene.cta} onChange={e => updateScene({ cta: e.target.value })}/>}</>} {/* Pitch metadata replaces the ordinary CTA. */}
    ...
    {matchPicker && <MatchPicker mode="fill" projectId={project.id} pageIndex={visibleIndex} notify={notify}
