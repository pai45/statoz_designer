...
export async function compositionHtml(project: Project, pageIndex = 0): Promise<string> {
  ...
  for (const id of new Set(project.pages.flatMap(p => [p.assetId, p.tabletAssetId, p.emblemA, p.emblemB]).filter(Boolean))) {
  ...
  const [bundle, tokens, css, appCreativeCss, logo, onest, orbitron] = await Promise.all([
    ...
    fs.readFile(path.join(root, "src/features/compositions/app-creatives.css"), "utf8"),
