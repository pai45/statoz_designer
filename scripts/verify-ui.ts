...
try {
  ...
  await page.getByRole("tab", { name: "Sports", exact: true }).click(); assert.equal(await page.locator(".template-card").count(), 6);
  ...
  assert.equal(await page.getByRole("tab", { name: "Uploads", exact: true }).getAttribute("aria-selected"), "true");
  await page.getByRole("tab", { name: "Player portraits", exact: true }).click();
  assert.ok(await page.locator(".asset-card").count() > 100);
  await page.getByRole("tab", { name: "Player portraits", exact: true }).press("Home");
  assert.equal(await page.getByRole("tab", { name: "Uploads", exact: true }).getAttribute("aria-selected"), "true");
  await page.getByRole("tab", { name: "Design kit", exact: true }).click();
  await page.locator(".design-kit-card").nth(6).waitFor();
  assert.equal(await page.locator(".design-kit-card").count(), 7);
  assert.equal(await page.locator(".import-dropzone").count(), 0);
  await page.screenshot({ path: path.join(output, "design-kit-desktop.png"), fullPage: true });
  await page.getByRole("tab", { name: "Brand", exact: true }).click();
  assert.deepEqual(await page.getByRole("tablist", { name: "Brand reference" }).getByRole("tab").allTextContents(), ["Overview", "Brand assets", "Colors", "Typography", "Shape & surface", "Actions", "Components", "Layout & motion", "Use cases", "Do & don't"]);
  ...
  await page.locator(".guide-swatch span", { hasText: "#5CDFFF" }).first().waitFor();
  await page.screenshot({ path: path.join(output, "brand-colors-desktop.png"), fullPage: true });
  await page.getByRole("tab", { name: "Typography", exact: true }).click();
  await page.locator(".guide-grid-table td", { hasText: "108px" }).first().waitFor();
  await page.getByRole("tab", { name: "Actions", exact: true }).click();
  assert.equal(await page.locator(".catalog-card").count(), 11);
  const stake = page.getByRole("group", { name: "Stake" });
  await stake.getByRole("button", { name: "Raise stake by 5" }).click();
  assert.match(await stake.textContent() ?? "", /30/);
  await page.getByRole("radio", { name: "Cricket" }).click();
  assert.equal(await page.getByRole("radio", { name: "Cricket" }).getAttribute("aria-checked"), "true");
  await page.screenshot({ path: path.join(output, "brand-actions-desktop.png"), fullPage: true });
  await page.getByRole("tab", { name: "Components", exact: true }).click();
  assert.equal(await page.locator(".catalog-card").count(), 8);
  await page.getByRole("tab", { name: "Stats", exact: true }).click();
  assert.equal(await page.getByRole("tab", { name: "Stats", exact: true }).getAttribute("aria-selected"), "true");
  await page.screenshot({ path: path.join(output, "brand-components-desktop.png"), fullPage: true });
  await page.getByRole("tab", { name: "Use cases", exact: true }).click();
  await page.getByRole("tab", { name: "Match posters & match stories", exact: true }).click();
  await page.locator(".guide-recipe", { hasText: "reported figures only" }).waitFor();
  await page.screenshot({ path: path.join(output, "brand-use-cases-desktop.png"), fullPage: true });
  await page.setViewportSize({ width: 412, height: 900 });
  for (const tab of ["Actions", "Components", "Use cases", "Layout & motion", "Typography", "Overview"]) {
    await page.getByRole("tab", { name: tab, exact: true }).click();
    assert.ok(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), `${tab} scrolls horizontally on mobile`);
  }
  await page.screenshot({ path: path.join(output, "brand-mobile.png"), fullPage: true });
  await page.getByRole("tab", { name: "Use cases", exact: true }).click();
  await page.screenshot({ path: path.join(output, "brand-use-cases-mobile.png"), fullPage: true });
  await page.getByRole("tab", { name: "Actions", exact: true }).click();
  await page.screenshot({ path: path.join(output, "brand-actions-mobile.png"), fullPage: true });
